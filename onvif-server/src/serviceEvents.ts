...
import { OnvifServer, ServiceMethodHandler } from "./serverMockup";
...
*/
function getDurationFromSeconds(seconds: number): string {
...
export class EventsService {
	...
	public readonly serviceName = "events";
	...
	private subscriptions = new Map<string, PullPointSubscription>();

	constructor(server: OnvifServer) {
		server.registerMethod(this.serviceName, 'createPullPointSubscription', (args: any) => this.createPullPointSubscription(args));
		server.registerMethod(this.serviceName, 'unsubscribe', (args: any, header: any) => this.unsubscribe(args, header));
		server.registerMethod(this.serviceName, 'pullMessages', async (args: any, header: any) => await this.pullMessages(args, header));	
		server.registerMethod(this.serviceName, 'renew', (args: any, header: any) => this.renew(args, header));	
		server.registerMethod(this.serviceName, 'getEventProperties', (args: any) => this.getEventProperties(args));	
	}

	private getSubscriptionIdFromToHeader(to: any) {
	...
	private renew(args: any, header: any): string {
	...
	private getEventProperties(args: any): string {

		console.log("getEventProperties(): " + util.inspect(args, false, null, true));
		return `
...
class PullPointSubscription {
	...
	constructor(id: string, expirationTime: Date) {
		this.id = id;
...
class Message {
	...
	constructor(type: string, timestamp: Date, payload: any) {
		this.type = type;
